...
fn build_compact_equation_system_v2(intervals: &Vec<GraphSplineInterval>) -> Vec<CompactRowV2> {
    let mut rows = Vec::new();

    let first_interval = intervals.first().expect("We should have at least one interval if we called this function");
    let mut row = CompactRowV2::new();
    row.add_entry(0, 1.0);
    row.rhs = first_interval.start.y;
    rows.push(row);
    ...
    intervals.windows(2).for_each(|intervals_pair| {
        ...
        let mut row = CompactRowV2::new();
        add_equation_factors_to_compact_row_v2(
            &mut row,
            coefficient_idx,
            intervals_pair[0].polynomial.coefficients.len(),
            max_x_value_left,
...
fn build_compact_equation_system_v5(intervals: &Vec<GraphSplineInterval>) -> Compact1DBandMatrix {
...
fn build_compact_equation_system_v8(intervals: &Vec<GraphSplineInterval>) -> Compact1DBandMatrix {
    let mut n: usize = 0;
    let mut bw: usize = 0;
...
pub fn get_graph_spline_interpolation_function_v7(points: &[Point]) -> Option<GraphSpline> {
    if points.len() < 2 {
        return None;
    }
...
pub fn get_graph_spline_interpolation_function_v8(points: &[Point]) -> Option<GraphSpline> {
    if points.len() < 2 {
        return None;
    }
...
pub fn get_graph_spline_interpolation_function_v10(points: &[Point]) -> Option<GraphSpline> {
    if points.len() < 2 {
        return None;
    }
