...
#[derive(Debug, PartialEq, Clone, Serialize)]
pub struct DistributionRational {
...
impl DistributionRational {
    pub fn constant(c: u32) -> DistributionRational {
        DistributionRational {
            probabilities: vec![Rational::from((1, 1))],
            min_value: c
        }
    }

    pub fn add(&mut self, other: & DistributionRational) {
        let cur_len = self.probabilities.len();

        self.probabilities.resize(cur_len + other.probabilities.len() - 1, Rational::from((0, 1)));
    ...
    pub fn merge(&mut self, other: & DistributionRational) {
        let new_min = self.min_value.min(other.min_value);
        ...
        let mut new_probs = vec![Rational::from((0, 1)); new_len];
        for (i, p) in self.probabilities.iter().enumerate() {
    ...
    pub fn clamped(&self, modifier: &ClampingModifier) -> DistributionRational {
        match modifier {
            ClampingModifier::Minimum { number } => {
                let n = *number;
                if n <= self.min_value { return self.clone(); }
                let idx = (n - self.min_value) as usize;
                if idx >= self.probabilities.len() { return DistributionRational::constant(n); }
                let mut probs = self.probabilities.clone();
                let clamped: Rational = probs[..idx].iter().sum();
                probs.drain(0..idx);
                probs[0] += clamped;
                DistributionRational { probabilities: probs, min_value: n }
            }
            ClampingModifier::Maximum { number } => {
                let n = *number;
                let max_val = self.min_value + self.probabilities.len() as u32 - 1;
                ...
                let idx = (n - self.min_value) as usize;
                let mut probs = self.probabilities.clone();
                let clamped: Rational = probs[idx + 1..].iter().sum();
                probs.truncate(idx + 1);
                probs[idx] += clamped;
                DistributionRational { probabilities: probs, min_value: self.min_value }
            }
        }
    }
    ...
    /// forcing all the possible permutation and enumerating all of them will be faster
    /// !todo("Write the brute force code and benchmark it against this one, find the sweet spot where the brute force becomes faster, and call the brute force when you know it's faster than the general case")
    pub fn get_sum_highest_k_distribution(&self, number_of_dice: u32, number_of_kept_dice: u32) -> DistributionRational {
        let number_of_dice_usize = number_of_dice as usize;
        let number_of_kept_dice_usize = number_of_kept_dice as usize;

        // Base: only the smallest face — all dice land on it
        let mut state_below_face = vec![vec![DistributionRational::constant(0); number_of_kept_dice_usize + 1]; number_of_dice_usize + 1];
        let v0 = self.min_value;
        for cur_number_of_dice in 0..=number_of_dice_usize {
            for cur_number_of_kept_dice in 0..=cur_number_of_dice.min(number_of_kept_dice_usize) {
                state_below_face[cur_number_of_dice][cur_number_of_kept_dice] = DistributionRational::constant(cur_number_of_kept_dice as u32 * v0);
            }
        }

        let mut prob_lower = self.probabilities[0].clone(); // cumulative probability of faces in state_below_face
        for idx in 1..self.probabilities.len() {
            let cur_face_value = self.min_value + idx as u32;
            let cur_face_prob = &self.probabilities[idx];

            let prob_total = (&prob_lower + cur_face_prob).complete();  // P(lower than or equal to this face)
            let mut state_up_to_face = vec![vec![DistributionRational::constant(0); number_of_kept_dice_usize + 1]; number_of_dice_usize + 1];
            for cur_number_of_dice in 0..=number_of_dice_usize {
                for cur_number_of_kept_dice in 0..=cur_number_of_dice.min(number_of_kept_dice_usize) {
...
pub fn get_dice_roll_distribution_rational(roll: & DiceRoll) -> DistributionRational {
    let mut res: Option<DistributionRational> = None;

    let base_dice_distribution = DistributionRational {
        probabilities: vec![Rational::from((1, roll.dice_size)); roll.dice_size as usize],
        min_value: 1
    };
    ...
    if let Some(modifier) = & roll.reroll_modifier {
        ...
        let mut bad_dice_distribution = DistributionRational {
            probabilities: Vec::with_capacity(roll.dice_size as usize),
            min_value: 0,
        };
        ...
        (1..=roll.dice_size)
            .for_each(|item| {
                if bad_rolls.contains(& item) {
                    ...
                    bad_dice_distribution.probabilities.push(Rational::from((1, bad_rolls.len())));

                    if good_dice_distribution.min_value > 0 {
                        good_dice_distribution.probabilities.push(Rational::from((0, 1)));
                    }
                } else {
                    if good_dice_distribution.min_value == 0 {
                        good_dice_distribution.min_value = item;
                    ...
                    good_dice_distribution.probabilities.push(Rational::from((1, roll.dice_size as usize - bad_rolls.len())));

                    if bad_dice_distribution.min_value > 0 {
                        bad_dice_distribution.probabilities.push(Rational::from((0, 1)));
                    }
                }
            })
        ;
        ...
        for number_of_bad_dice in 0..=roll.number_of_dice {
            ...
            let scenario_distribution =
                match &roll.keeping_result_modifier {
                    Some(KeepingResultModifier::KeepHighest { number_of_dice }) => {
                        compute_scenario_keep_n_rational(
                            *number_of_dice, true,
                            number_of_good_dice, &good_dice_distribution,
                            number_of_bad_dice, number_of_kept_bad_dice,
                            number_of_rerolled_dice, &bad_dice_distribution,
                    ...
                    None => {
                        compute_scenario_no_keep_rational(
                            number_of_good_dice, &good_dice_distribution,
                            number_of_bad_dice, number_of_kept_bad_dice,
                            number_of_rerolled_dice, &bad_dice_distribution,
                            &base_dice_distribution,
        ...
        match &roll.keeping_result_modifier {
            ...
            Some(KeepingResultModifier::KeepLowest { number_of_dice }) => {
                let k = (*number_of_dice).min(roll.number_of_dice);
                let rev_base = reverse_distribution_rational(&effective_base, roll.dice_size);
                let rev_result = rev_base.get_sum_highest_k_distribution(roll.number_of_dice, k);
                let max_val = rev_result.min_value + rev_result.probabilities.len() as u32 - 1;
                let mut probs = rev_result.probabilities.clone();
                probs.reverse();
            ...
            None => {
                if roll.clamping_modifier.is_none() {
...
fn compute_scenario_no_keep_rational(
    number_of_good_dice: u32,
    good_dist: &DistributionRational,
    number_of_bad_dice: u32,
    number_of_kept_bad_dice: u32,
    number_of_rerolled_dice: u32,
    bad_dist: &DistributionRational,
    base_dist: &DistributionRational,
) -> DistributionRational {
    let mut dist = DistributionRational {
        probabilities: vec![Rational::from((1, 1))],
        min_value: 0,
    };
...
fn compute_scenario_keep_n_rational(
    k: u32,
    descending: bool,
    number_of_good_dice: u32,
    good_dist: &DistributionRational,
    ...
    let mut state: HashMap<Vec<u32>, Rational> = HashMap::new();
    state.insert(Vec::new(), Rational::from((1, 1)));

    fn add_dice_to_state(
        state: HashMap<Vec<u32>, Rational>,
        dist: &DistributionRational,
        count: u32,
        k: usize,
        descending: bool,
    ) -> HashMap<Vec<u32>, Rational> {
        let values: Vec<u32> = (dist.min_value..dist.min_value + dist.probabilities.len() as u32).collect();
        let mut current = state;
        for _ in 0..count {
            let mut next: HashMap<Vec<u32>, Rational> = HashMap::new();
            for (top_vals, prob) in &current {
                for (vi, &v) in values.iter().enumerate() {
                    let p_die = &dist.probabilities[vi];
                    let mut new_vals = top_vals.clone();
                    ...
                    new_vals.truncate(k);
                    *next.entry(new_vals).or_insert(Rational::from((0, 1))) += (prob * p_die).complete();
                }
            }
            current = next;
        }
    ...
    if kept_bad_to_model > 0 {
        // Bad dice selection always keeps HIGHEST (original values), regardless of final keep direction
        let mut bad_state: HashMap<Vec<u32>, Rational> = HashMap::new();
        bad_state.insert(Vec::new(), Rational::from((1, 1)));
        bad_state = add_dice_to_state(bad_state, bad_dist, number_of_bad_dice, kept_bad_to_model as usize, true);

        let mut merged: HashMap<Vec<u32>, Rational> = HashMap::new();
        for (main_vals, main_prob) in &state {
            for (bad_vals, bad_prob) in &bad_state {
                let mut combined = main_vals.clone();
                combined.extend(bad_vals.iter().copied());
                ...
                combined.truncate(k);
                *merged.entry(combined).or_insert(Rational::from((0, 1))) += (main_prob * bad_prob).complete();
            }
        }
        state = merged;
    }
    ...
    // Apply per-die clamping
    if let Some(clamp) = clamp {
        let mut clamped_state: HashMap<Vec<u32>, Rational> = HashMap::new();
        for (vals, prob) in &state {
            let clamped_vals: Vec<u32> = vals.iter().map(|&v| match clamp {
                ClampingModifier::Minimum { number } => v.max(*number),
                ClampingModifier::Maximum { number } => v.min(*number),
            }).collect();
            *clamped_state.entry(clamped_vals).or_insert(Rational::from((0, 1))) += prob;
        }
        state = clamped_state;
    }

    let mut sum_probs: HashMap<u32, Rational> = HashMap::new();
    for (vals, prob) in &state {
        let sum: u32 = vals.iter().sum();
        *sum_probs.entry(sum).or_insert(Rational::from((0, 1))) += prob;
    }

    let min_sum = sum_probs.keys().min().copied().unwrap_or(0);
    let max_sum = sum_probs.keys().max().copied().unwrap_or(0);
    let mut probs = vec![Rational::from((0, 1)); (max_sum - min_sum + 1) as usize];
    for (s, p) in sum_probs {
        probs[(s - min_sum) as usize] = p;
    }

    DistributionRational { probabilities: probs, min_value: min_sum }
}

fn reverse_distribution_rational(dist: &DistributionRational, dice_size: u32) -> DistributionRational {
    let max_v = dice_size;
    let original_max = dist.min_value + dist.probabilities.len() as u32 - 1;
    let new_min = max_v + 1 - original_max;
    let mut probs = dist.probabilities.clone();
    probs.reverse();
    DistributionRational { probabilities: probs, min_value: new_min }
}
